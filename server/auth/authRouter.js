...
const { bidchuckApiUrl, bidchuckClientSecret } = dotenv.parsed
...
router.post('/register', (req, res) => {
  ...
  axios
    .post(`${bidchuckApiUrl}/users`, {
      email: req.body.email,
      password: hashedPassword,
    })
    .then(({ data }) => {
      const token = jwt.sign({ id: data.id }, bidchuckClientSecret, {
...
router.get('/me', VerifyToken, (req, res) => {
  axios
    .get(`${bidchuckApiUrl}/users/${req.userId}`)
    .then(response => {
  ...
  axios
    ...
    .then(response => {
      ...
      if (user && req.body.password) {
        ...
        const token = jwt.sign({ id: user.id }, bidchuckClientSecret, {
          expiresIn: 86400, // expires in 24 hours
        })
